...
lines3d(trend, col = "red3", lwd = 3)
cols = colorRampPalette(c("lightblue", "firebrick"))(c * k + 1)
points3d(dta, col = cols, size = 5)

mu0 = dta[1,]
Q0 = sphere_diff_core(dta[10,], dta[1,])$Q

pred_init = sphere_trend(c(1:(c * k)) / k, Q0, mu0)
cols = colorRampPalette(c("lightblue", "lightgreen"))(c * k + 1)
lines3d(pred_init, col = cols, lwd = 5)

model = spt(dta, 1, Q0, mu0, alpha_Q = 0.01, alpha_mu = 0.001, x_exact = FALSE,
            max.iter = 1000, verbose = TRUE)
pred = sphere_trend(c(1:(c * k)) / k, model$Q, model$mu)
cols = colorRampPalette(c("lightgreen", "green"))(c * k + 1)
lines3d(pred, col = cols, lwd = 5)
points3d(model$mu, col = "lightblue", size = 10)
...
lines3d(pred, col = cols, lwd = 5)
points3d(model$mu, col = "blue", size = 10)

model = spt(dta, 1, Q0, mu0, alpha_Q = 0.01, alpha_mu = 0.001, x_exact = FALSE,
            max.iter = 10000, verbose = TRUE)
pred = sphere_trend(c(1:(c * k)) / k, model$Q, model$mu)
cols = colorRampPalette(c("lightgreen", "gold"))(c * k + 1)
lines3d(pred, col = cols, lwd = 5)
